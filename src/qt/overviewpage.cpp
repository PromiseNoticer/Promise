...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizepromiseAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizepromiseAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizepromiseAmount = strAnonymizepromiseAmount.remove(strAnonymizepromiseAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizepromiseAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizepromiseAmount*COIN) nMaxToAnonymize = nAnonymizepromiseAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizepromiseAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizepromiseAmount));
        strAnonymizepromiseAmount = strAnonymizepromiseAmount.remove(strAnonymizepromiseAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizepromiseAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
