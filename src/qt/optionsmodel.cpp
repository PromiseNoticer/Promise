...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizepromiseAmount"))
        settings.setValue("nAnonymizepromiseAmount", 1000);
    nAnonymizepromiseAmount = settings.value("nAnonymizepromiseAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizepromiseAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizepromiseAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizepromiseAmount:
            return QVariant(nAnonymizepromiseAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizepromiseAmount:
            nAnonymizepromiseAmount = value.toInt();
            settings.setValue("nAnonymizepromiseAmount", nAnonymizepromiseAmount);
            emit AnonymizepromiseAmountChanged(nAnonymizepromiseAmount);
            break;
        default:
            break;
        }
